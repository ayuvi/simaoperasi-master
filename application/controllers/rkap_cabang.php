...
class rkap_cabang extends CI_Controller
{
    ...
    public function ax_data_rkap()
    {
        ...
        if ($this->session->userdata('login')) {
            ...
            if (!empty($access['id_menu_details'])) {
                ...
                $data = $this->model_rkap->getAllrkapCabang($length, $start, $cari['value'], $id_segment,$id_coa)->result_array();
                $count = $this->model_rkap->get_count_rkap_cabang($cari['value'],$id_segment,$id_coa);
...
{
    $data = array(
        $this->input->post('column_name')  =>  $this->input->post('value') ? $this->input->post('value') : 0
    );
    $this->db->where('id_rkap', $this->input->post('id'));
