<?php
class Model_jadwal_borongan_log extends CI_Model
{
	...
	public function getAllrelasi($show=null, $start=null, $cari=null)
	{
		...
		$this->db->where("(a.nm_relasi LIKE '%".$cari."%' OR a.asal LIKE '%".$cari."%' OR a.tujuan LIKE '%".$cari."%' OR a.rate LIKE '%".$cari."%' ) ");
	...
	public function getAllsewa($show=null, $start=null, $cari=null,$id_jadwal_borongan)
	{
		$this->db->select("a.*");
		$this->db->from("ms_sewa_borongan a");
		$session = $this->session->userdata('login');
		$this->db->where("(a.tanggal LIKE '%".$cari."%' OR a.bayar LIKE '%".$cari."%') ");
		$this->db->where("a.id_jadwal_borongan",$id_jadwal_borongan);
		if ($show == null && $start == null) {
		} else {
			$this->db->limit($show, $start);
		}
	...
	public function get_count_sewa($cari = null,$id_jadwal_borongan)
	{
		$count = array();
		$session = $this->session->userdata('login');
		
		$this->db->select(" COUNT(id_sewa_borongan) as recordsFiltered ");
		$this->db->from("ms_sewa_borongan");
		$this->db->where("(tanggal LIKE '%".$cari."%' OR bayar LIKE '%".$cari."%') ");
		$this->db->where("id_jadwal_borongan",$id_jadwal_borongan);
		$count['recordsFiltered'] = $this->db->get()->row_array()['recordsFiltered'];
		
		$this->db->select(" COUNT(id_sewa_borongan) as recordsTotal ");
		$this->db->from("ms_sewa_borongan");
		$count['recordsTotal'] = $this->db->get()->row_array()['recordsTotal'];
   ...
   //          return $this->db->get();
	...
	public function get_relasi($id_relasi)
	{
		$session = $this->session->userdata('login');
		$this->db->select("a.*");
		$this->db->from("ref_relasi a");
