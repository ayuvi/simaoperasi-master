	...
	<div class="wrapper">
		...
		<script type='text/javascript'>
			var absenTable = $('#absenTable').DataTable({
				...
				ajax: 
				{
					url: "<?= base_url()?>absensi_teknik/ax_data_absensi_teknik/",
					type: 'POST',
					data: function ( d ) {
						return $.extend({}, d, { 
				...
				{ class: "intro",data: "status", render: function(data, type, full, meta){
					if(data == 1){
						...
						return 'Rusak Ringan (RR)';
					}else if(data == 3){
						return 'Service (S)';
					}else if(data == 4){
						return 'Siap Guna Operasi (SGO)';
					}else{
						return '';
					}
				}},
			...
			function print_pdf() {
				...
				if (tanggal != '' && id_cabang != '') {
					...
					var REQ = "?bulan="+bulan+"&tahun="+tahun+"&tanggal="+tanggal+"&id_cabang="+id_cabang+"&format=html"+"&uk=F4-P"+"&name=absensi_armada_teknik";
					open(url+REQ);
				}else{
