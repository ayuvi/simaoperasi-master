	...
	<body class="sidebar-mini wysihtml5-supported <?= $this->config->item('color')?>">
										...
										<div class="modal-dialog">
											<div class="modal-content">
												<div class="modal-header">
													...
													<h4 class="Form-add-bu" id="addModalLabel">Form Add project</h4>
												</div>
		...
		<script type='text/javascript'>
			var buTable = $('#buTable').DataTable({
				...
				ajax: 
				{
					url: "<?= base_url()?>project/ax_data_project/",
					type: 'POST'
				},
				columns: 
				[
			...
			$('#btnSave').on('click', function () {
				...
				{
					var url = '<?=base_url()?>project/ax_set_data';
					var data = {
			...
			function ViewData(id_project)
			{
				if(id_project == 0)
				{
					$('#addModalLabel').html('Add project');
					$('#id_project').val('');
					$('#nm_project').val('');
					$('#active').val('1');
					$('#addModal').modal('show');
				...
				{
					var url = '<?=base_url()?>project/ax_get_data_by_id';
					var data = {
					...
					$.ajax({
						...
						var data = JSON.parse(data);
						$('#addModalLabel').html('Edit project');
			...
			function DeleteData(id_project)
			{
				alertify.confirm(
					...
					function(){
						var url = '<?=base_url()?>project/ax_unset_data';
						var data = {
							id_project: id_project
						};
