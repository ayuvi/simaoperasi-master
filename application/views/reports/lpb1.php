...
<body class="sidebar-mini wysihtml5-supported <?= $this->config->item('color')?>">
	...
	<script type="text/javascript">
		...
		function printCetak() {
			...
			{
				alertify.alert("Warning", "Tahun Belum di Isi.");
			}else if($('#bulan').val() == '' ||$('#bulan').val() == '0')
			{
				alertify.alert("Warning", "Bulan Belum di Pilih.");
			}else if($('#id_segment').val() == '' || $('#id_segment').val() == null)
			{
				alertify.alert("Warning", "Segment Belum di Pilih.");
			}else{
				var REQ = "?id_segment="+id_segment+"&tahun="+tahun+"&bulan="+bulan+"&tanggal="+tanggal+"&format="+format+"&uk=F4-L"+"&name=lpb1";
				open(url+REQ);
			}
		}
	</script>
