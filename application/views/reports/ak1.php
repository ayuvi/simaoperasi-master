								...
								<form id="form" class="form-horizontal">
									<div class="form-body">
										...
										<div class="form-group" id="div_penerimaan">
											<label class="control-label col-md-2">Penerimaan</label>
											<div class="col-md-9">
												<select data-placeholder="-- Pilih Penerimaan --" class="form-control select2" style="width: 100%;" id="penerimaan">
													<option value="">-- Pilih Penerimaan --</option>
													<?php 
													$arr=1;
													foreach ($combobox_komponen_pendapatan->result() as $rowmenu) { ?>
													<option value="<?= $rowmenu->id_komponen?>"><?=$arr.". ".$rowmenu->nm_komponen?></option>
													<?php $arr+=1; } ?>
												</select>
											</div>
										</div>
										<!-- <div class="form-group has-success" id="div_armada">
	...
	<script type="text/javascript">
		function set_display() {
			...
			if (x==1) {
				...
				document.getElementById('div_penerimaan').style.display = 'block';
				...
				document.getElementById('div_segment').style.display = 'block';
				document.getElementById('div_trayek').style.display = 'block';
				document.getElementById('div_penerimaan').style.display = 'none';
				document.getElementById('div_tanggal').style.display = 'block';
		...
		function armadalist(){
			$.ajax({
				type: "POST", 
				url: "<?= base_url() ?>reports/ax_get_armada_ak1_dan_ak2",
				data: {id_bu : $("#id_bu").val(), id_segment : $("#id_segment").val()}, 
				dataType: "json",
				beforeSend: function(e) {
					if(e && e.overrideMimeType) {
				...
				success: function(response){ 
					...
					$('#select2-id_armada-container').html('--All Armada--');
					$('#id_armada').val('0');
				},
				error: function (xhr, ajaxOptions, thrownError) { 
					alert(thrownError); 
		...
		function printCetak() {
			...
			if(x==0){
				...
				}else if($('#penerimaan').val() == '')
				{
					alertify.alert("Warning", "Penerimaan Belum di Pilih.");
				}else if($('#tanggal').val() == '')
				{
					alertify.alert("Warning", "Tanggal Belum di Isi.");
				}else{
					var REQ = "?id_segment="+id_segment+"&id_bu="+id_bu+"&kd_trayek="+kd_trayek+"&penerimaan="+penerimaan+"&tanggal="+tanggal+"&kontrol="+kontrol+"&combo="+x+"&format="+format+"&uk=F4-L"+"&name=ak1_new";
					open(url+REQ);
				}
