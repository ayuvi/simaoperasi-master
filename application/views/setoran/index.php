											...
											<div class="box box-primary box-solid">
												...
												<div class="box-body">
													<div class="modal-body">
														...
														<input type="hidden" id="tgl_setoran_header" name="tgl_setoran_header" class="form-control" placeholder="Tanggal Setoran" readonly="true">
											...
											<div class="box box-primary box-solid">
												...
												<div class="box-body no-padding">
													<div class="modal-body">
														...
														<input type="hidden" id="kd_segmen_pnp" name="kd_segmen_pnp" class="form-control" placeholder="Kode Segmen" readonly="true">
													...
													<div class="modal-body">
														...
														<div class="row">
															...
															<div class="form-group col-lg-4" id="div_pendapatan_jumlah">
																<label id="label_jumlah_pend">Jumlah</label>
...
<script type='text/javascript'>
	...
	function on_off_pendapatan() {
		...
		if(id_jenis_pend==6){
			...
			$("#jumlah_pend").val('');
			$("#label_jumlah_pend").html('Ritase');
			harga_rp_subsidi($("#id_bu_filter").val(),$("#id_trayek_pnp").val());
			// $("#harga_pend").val('');
		}else{
			document.getElementById('div_pendapatan_jumlah').style.display = 'block';
			document.getElementById('div_pendapatan_harga').style.display = 'block';
	...
	var setoranTable = $('#setoranTable').DataTable({
		...
		buttons: [
					...
					{ data: "id_setoran", render: function(data, type, full, meta){
						var id = "'"+full['id_setoran']+"','"+full['armada']+"','"+full['id_bu']+"','"+full['tgl_setoran']+"'";
	...
	var setoranTableDetail = $('#setoranTableDetail').DataTable({
		...
		buttons: [
					...
					{ data: "id_setoran_detail", render: function(data, type, full, meta){
						...
						var id4 = "'"+full['id_setoran_detail']+"','"+full['id_setoran']+"','"+full['armada']+"','"+full['tanggal']+"','"+full['id_bu']+"','"+full['id_jadwal']+"','"+full['status_udj']+"','"+full['status_udj_bagasi']+"','"+full['kd_trayek']+"','"+full['id_trayek']+"','"+full['id_layanan']+"','"+full['kd_segmen']+"'";
						...
						var data_wisma = ["WATL-BSH", "WAK-BSH"];
						if(full['id_bu']==3 && data_wisma.includes(full['kd_trayek'])){
							str += '<a type="button" class="btn btn-sm btn-success" onclick="printSPSAB(' + id4 + ')"><i class="fa fa-print"></i> SPSAB</a>';
						}
	...
	var setoranTableDetailPng = $('#setoranTableDetailPng').DataTable({
		...
		buttons: [
					...
					{ data: "id_setoran_pnp", render: function(data, type, full, meta){
						var str = '';
						str += '<div class="btn-group">';
						// str += '<a type="button" class="btn btn-sm btn-info" onclick="pertelaanPng(' + data + ')"><i class="fa fa-list-alt"></i> </a>';
						str += '<a type="button" class="btn btn-sm btn-primary" onclick="editPng(' + data + ')"><i class="fa fa-pencil"></i> </a>';
						str += '<a type="button" class="btn btn-sm btn-danger" onclick="hapusPng(' + data + ')"><i class="fa fa-trash"></i> </a>';
						str += '</div>';
						return str;
					}},
					{ data: "kd_trayek" },
	...
	$('#btnSaveDetail').on('click', function () {
		...
		}else if($('#tgl_keberangkatan').val() > $('#tgl_setoran_header').val()){
			alertify.alert("Warning", "Tanggal Jadwal : "+$('#tgl_keberangkatan').val()+", Sedangkan tanggal setoran : "+$('#tgl_setoran_header').val()+". Data tidak berhasil disimpan");
		}else {
	...
	function tambahDetail(id_setoran,armada,id_bu,tgl_setoran){
		if(id_setoran == 0){
						...
						$.ajax({
							...
							$('#tgl_setoran_header').val(tgl_setoran);
				...
				function tambahPng(id_setoran_detail,id_setoran,kd_armada,tanggal,id_bu,id_jadwal,status_udj,status_udj_bagasi,kd_trayek,id_trayek,id_layanan,kd_segmen){
					combo_trayek_pnp(id_bu,id_trayek,id_layanan,kd_segmen);
					...
					//tambah option wisma atlet
					var data_trayek = ["WATL-BSH", "WAK-BSH"];
					if(data_trayek.includes(kd_trayek)){
						var sel = document.getElementById('jenis_penjualan_pnp');
						var opt = document.createElement('option');
						opt.appendChild( document.createTextNode('Shuttle'));
						opt.value = '7'; 
						sel.appendChild(opt);
					}
					...
					harga_rp_km(id_bu,id_trayek);
					harga_rp_subsidi(id_bu,id_trayek);
					document.getElementById('div_pendapatan_jumlah').style.display = 'block';
					document.getElementById('div_pendapatan_harga').style.display = 'block';
					document.getElementById('div_pendapatan_km_tempuh').style.display = 'none';
					document.getElementById('div_pendapatan_rp_km').style.display = 'none';
			...
			function combo_trayek_pnp(id_bu,id_trayek,id_layanan,kd_segmen){
				$.ajax({
					type: "POST", 
			...
			function harga_rp_km(id_bu,id_trayek){
			...
			function harga_rp_subsidi(id_bu,id_trayek){
				...
				$.ajax({
					...
					var data = JSON.parse(data);
					$('#harga_pend').val(parseInt(data['rp_subsidi']));
				});
			}
			...
			function printSPSAB(id_setoran_detail,id_setoran,armada,tanggal,id_bu,id_jadwal,status_udj,status_udj_bagasi,kd_trayek,id_trayek,id_layanan,kd_segmen){
				var url 		= "<?=site_url("reports/prints")?>";
				var REQ = "?id_setoran_detail="+id_setoran_detail+"&id_setoran="+id_setoran+"&armada="+armada+"&id_bu="+id_bu+"&id_jadwal="+id_jadwal+"&format=pdf"+"&uk=A4-P"+"&name=spsab_wisma_atlet";
				open(url+REQ);
			}
