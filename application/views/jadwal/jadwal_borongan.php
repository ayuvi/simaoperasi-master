...
<script type="text/javascript">
	...
	function setPembayaran(id_jadwal,upp,bayar){
		var url = '<?=base_url()?>jadwal_borongan/ax_get_data_by_id_jadwal';
		var data = {
			id_jadwal: id_jadwal
		};
	...
	$('#btnSaveUPP').on('click', function () {
		if($('#upp').val()=='0'){
			...
			var url = '<?=base_url()?>jadwal_borongan/ax_set_data_upp';
			var data = {
	...
	var tbljadwalnondefault = $('#tbljadwalnondefault').DataTable({
		...
		ajax: 
		{
			url: "<?= base_url()?>jadwal_borongan/ax_data_jadwal_hr/",
			type: 'POST',
			data: function ( d ) {
				return $.extend({}, d, { 	
	...
	function ViewDetail(unik,hr){
		...
		if($('#id_cabang').val() == ''){
			...
			if(unik == 0){
				...
				var url = '<?=base_url()?>jadwal_borongan/ax_get_data_by_id_detail';
	...
	function DeleteData(id_jadwal){
		alertify.confirm(
			...
			function(){
				var url = '<?=base_url()?>jadwal_borongan/ax_unset_data';
				var data = {
					id_jadwal: id_jadwal
				};
	...
	$('#btnSave').on('click', function () {
		if($('#nama').val()==''){
			...
			var url = '<?=base_url()?>jadwal_borongan/ax_set_data_default';
			var tgl = $('#tanggal').val();		
			var data = {
	...
	function setRitasearmada(armada){
		if($('#kd_segmen').val() == 0){
			...
			alertify.confirm(
				...
				function(){
					var url = '<?=base_url()?>jadwal_borongan/setRitasearmada';
					var data = {
						id_jadwal	: armada
					};
					$.ajax({
	...
	function checkoutabsen(id_lmb){
		if(id_lmb == 0){
			...
			alertify.confirm(
				...
				'CheckOut Absensi?', 
				function(){
					var url = '<?=base_url()?>jadwal_borongan/checkoutabsen';
					var data = {
						id_lmb	: id_lmb,
					};
					$.ajax({
	...
	function poollist(){
		$.ajax({
			type: "POST", 
			url: "<?= base_url() ?>jadwal_borongan/ax_get_pool", 
	...
	function driverlist(){
		$.ajax({
			type: "POST", 
			url: "<?= base_url() ?>jadwal_borongan/ax_get_driver", 
	...
	function armadalist(){
		$.ajax({
			type: "POST", 
			url: "<?= base_url() ?>jadwal_borongan/ax_get_armada", 
			data: {id_cabang : $("#id_cabang").val(),tgl_absensi : $("#tanggal").val()}, 
			dataType: "json",
			beforeSend: function(e) {
				if(e && e.overrideMimeType) {
	...
	function deleteAllJadwal(){
		if($('#id_cabang').val() == ''){
			...
			alertify.confirm(
				...
				function(){
					var url = '<?=base_url()?>jadwal_borongan/ax_unset_data_all_jadwal';
					var data = {
						id_bu: id_bu,
						id_pool: id_pool,
						kd_segmen: kd_segmen,
	...
	$('#btnCopy').on('click', function () {
		if($('#tanggal_to').val()==''){
			...
			alertify.confirm(
				...
				function(){
					var url = '<?=base_url()?>jadwal_borongan/ax_copy_jadwal';
					var data = {
						kd_segmen	: kd_segmen,
						id_cabang	: id_cabang,
						id_pool		: id_pool,
	...
	function seat_armada() {
		var url = '<?=base_url()?>jadwal_borongan/ax_get_seat_armada';
		var data = {
			id_bu 		: $('#id_cabang').val(),
		...
		$.ajax({
			...
			var data = JSON.parse(data);
			$('#jum_penumpang').val(data['seat_armada']);
		});
	}
