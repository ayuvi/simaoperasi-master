...
<head>
	<?= $this->load->view('head'); ?>
	<link rel="stylesheet" type="text/css" href="<?php echo base_url('dropzone/dropzone.min.css') ?>">
	<link rel="stylesheet" type="text/css" href="<?php echo base_url('dropzone/basic.min.css') ?>">

	<script type="text/javascript" src="<?php echo base_url('dropzone/jquery.js') ?>"></script>
	<script type="text/javascript" src="<?php echo base_url('dropzone/dropzone.min.js') ?>"></script>
	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
			...
			<section class="content-header">
				<h1>ARMADA DRAG N DROP PHOTOS</h1>
			</section>
											...
											<div class="row">
												<form id="submit">
													...
													<div class="form-group col-lg-6">
														...
														<input type="file" name="file" id="file_attachment" accept=".jpeg,.jpg" class="form-control">
		...
		<script type='text/javascript'>
			var armadaTable = $('#armadaTable').DataTable({
				...
				buttons: [
					...
					ajax: 
					{
						url: "<?= base_url()?>armada_drag/ax_data_armada_drag/",
						type: 'POST',
						data: function ( d ) {
							return $.extend({}, d, { 
			...
			$('#btnSave').on('click', function () {
				...
				{
					var url = '<?=base_url()?>armada_drag/ax_set_data';
					var data = {
			...
			function ViewData(id_armada)
			{
				...
				{
					var url = '<?=base_url()?>armada_drag/ax_get_data_by_id';
					var data = {
						id_armada: id_armada
					};
			...
			function DeleteData(id_armada)
			{
				alertify.confirm(
					...
					function(){
						var url = '<?=base_url()?>armada_drag/ax_unset_data';
						var data = {
							id_armada: id_armada
						};
			...
			var attachmentTable = $('#attachmentTable').DataTable({
				...
				ajax: 
				{
					url: "<?= base_url()?>armada_drag/ax_data_foto/",
					type: 'POST',
					data: function ( d ) {
						return $.extend({}, d, { 
...

				{
					data: "id_armada_foto", render: function(data, type, full, meta){
						var str = '';
						str += '<div class="btn-group">';
						str += '<a type="button" class="btn btn-sm btn-danger" title="Delete" onclick="DeleteFoto(' + data + ')"><i class="fa fa-trash"></i> </a>';
						str += '<a type="button" class="btn btn-sm btn-info"  onclick="ubahSisi(' + data + ','+ "'DEPAN KANAN'" +')">D-Kanan</a>';
						str += '<a type="button" class="btn btn-sm btn-primary"  onclick="ubahSisi(' + data + ','+ "'DEPAN KIRI'" +')">D-Kiri</a>';
						str += '<a type="button" class="btn btn-sm btn-warning" onclick="ubahSisi(' + data + ','+ "'BELAKANG'" +')">Belakang</a>';
						str += '<a type="button" class="btn btn-sm bg-purple" onclick="ubahSisi(' + data + ','+ "'DALAM'" +')"> Dalam</a>';
						str += '</div>';
						return str;
					}
				},
			...
			function ubahSisi(userId, status)
			{
				var url = "<?php echo site_url('armada_drag/ubahSisi')?>";
				var data  = {
					id_armada_foto	: userId,
					status		: status
				};
				$.ajax({
					url : url,
			...
			$('#submit').submit(function(e){
				...
				{
					$.ajax({
						url:'<?php echo base_url();?>armada_drag/ax_upload_data_foto',
						type:"post",
			...
			function DeleteFoto(id_armada_foto)
			{
				alertify.confirm(
					...
					'Are you sure you want to delete this data?', 
					function(){
						var url = '<?=base_url()?>armada_drag/ax_unset_foto';
						var data = {
		...
		<script type="text/javascript">
			...
			var foto_upload= new Dropzone(".dropzone",{
				url: "<?php echo base_url('armada_drag/proses_upload') ?>",
				// maxFilesize: 2,
				// maxFiles: 2,
			...
			foto_upload.on("sending",function(a,b,c){
				...
				c.append("id_armada",$('#id_armada').val());
				alertify.success('Attachment Uploaded.');
			});

			foto_upload.on("queuecomplete", function(file) { 
				foto_upload.removeAllFiles(); 
				attachmentTable.ajax.reload();
			});
			...
			// 	$.ajax({
			// 		type:"post",
			// 		data:{id_armada:$('#id_armada').val()},
			// 		url:"<?php echo base_url('armada_drag/remove_foto') ?>",
			// 		cache:false,
			// 		dataType: 'json',
			// 		success: function(){
