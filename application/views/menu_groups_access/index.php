  ...
  <body class="sidebar-mini wysihtml5-supported <?= $this->config->item('color')?>">
    ...
    <script type='text/javascript'>
		function konfirmasi(data){	
			...
			if(x){
				location.href='<?= base_url()?>menu_groups_access/delete/'+data;
				}else{
    ...
    $(document).ready(function() {
        $('#dataTables-example').DataTable({
			...
			"serverSide": true,
			ajax: {
				url: "/menu_groups_access/ax_data_menu_groups_access/",
				type: 'POST'
			},
			columns: [
				{
					data: "id_menu_groups_access", render: function(data, type, full, meta)
					{						
						//return "<a href='#' onClick='konfirmasi("+data+")' class='btn btn-danger' data-toggle='tooltip' title='Delete'><i class='fa fa-trash'></i></a> <a href='menu_groups_access/formupdate/"+data+"' class='btn btn-warning' data-toggle='tooltip' title='Edit'><i class='fa fa-pencil'></i></a>";
						//return'<div class="btn-group"><button type="button" class="btn btn-default btn-sm">Action</button><button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="caret"></span><span class="sr-only">Toggle Dropdown</span></button><ul class="dropdown-menu"><li><a href="menu_groups_access/formupdate/'+data+'"><i class="fa fa-pencil"></i> Edit</a></li><li><a type="button" onClick="konfirmasi('+data+')"><i class="fa fa-trash"></i> Delete</a></li></ul></div>';
